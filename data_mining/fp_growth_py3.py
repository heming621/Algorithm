...
'''REF: https://github.com/enaeseth/python-fp-growth/blob/master/fp_growth.py'''
"""
...
Basic usage of the module is very simple:
    ...
    $ python fp_growth.py data/numeric.csv
"""
...
if __name__ == '__main__':
    ...
    # print "The transaction is:%s"%transactions
    ## 1, Transactions Generated
    transactions = dict()
    fr_recds = open(args[0], 'r')
    for line in fr_recds:
        times,ips = line.split(',')[0], ast.literal_eval(line[line.find(',')+1:])
        print(times, len(ips), type(times), type(ips))  # type: times(str), ips(list)
        transactions[times] = ips
    fr_recds.close()
    trans_ips = []
    for ips in transactions.values():
        trans_ips.append(ips)
    ## 2, Frequent Items
    start = time.clock()
    result = []
    for itemset, support in find_frequent_itemsets(trans_ips, options.minsup, True):
        result.append((itemset,support))
    result = sorted(result, key=lambda i: i[0])
    ## 3, Result output
    print("result:%s, len(result):%s"%(result, len(result)))
    fw_result = open('./data/freq.csv','w')
    for itemset, support in sorted(result,key=lambda i:len(i[0])): #result:
        fw_result.write("rlt: %s, %s/%s\n"%(itemset, support, len(transactions)))
        # print str(itemset) + ' ' + str(support)
    fw_result.close()
    print("run time: %f s" % (time.clock()-start))
